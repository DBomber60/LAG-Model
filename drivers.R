...
# make_pi generates the PI matrix described in the text above
make_pi = function(betahat, X1, cj, CO) {
  p = length(betahat)
  eta = as.numeric(X1 %*% betahat) # cg x 1 vector
  
  PI = array(0, dim=c(p,p))
  pi = (t(X1) %*% exp(eta))/gf(betahat,X1) # pi vector
  ...
  for (i in 1:p) {
    for(j in 1:p) {
      if(i==j) {
        ...
        z = which(cj[,i]==1 & cj[,j]==1) # in which cliques do covariate i and j co-occur
        coefficients = apply(matrix(X1[z,c(i,j)], nrow=length(z)), 1, prod) # what are the coefficients from the design matrix
        PI[i,j] = (exp(eta[z]) %*% coefficients)/gf(betahat,X1)
      }  
    }	
  }
  #rm(i,j)
...
# make_W generates the W matrix descibed above 
make_W = function(betahat, PI, X1, CO) {
  p = length(betahat)
  eta = as.numeric(X1 %*% betahat) # cg x 1 vector
  pi2 = (t(X1^2) %*% exp(eta))/gf(betahat,X1) # pi vector
  W = array(0, dim=c(p,p))
  for (i in 1:p) {
    for(j in 1:p) {
      {W[i,j]= (PI[i,j]-PI[i,i]*PI[j,j])} #if(CO[i,j]>0) # which cliques do they occur #
    }	
  }
  #rm(p)
  diag(W)=pi2-(diag(PI))^2
  return(W)
}


NRfit = function(S, X1, k, tol = 1e-6) {
  beta = rep(0,dim(X1)[2]) # initialize beta
  lhoodarray = lhood(S,beta,k,X1)
  loglik = lhood(S,beta,k,X1)
  
  # some variables that are inputs for making the PI and W matrices - only need to be computed once
  cj = t(apply(X1,1, function(x) ifelse(x>0,1,0)))
  CO = t(cj) %*% cj # co-occurence matrix 
  
  repeat { 
    PI = make_pi(beta, X1, cj, CO)
    W = make_W(beta, PI, X1, CO) + diag(rep(.001,dim(PI)[1]))
    #print(loglik)
    U = chol(W)
    Uinv = solve(U) 
    Winv = Uinv %*% t(Uinv)
    beta_tilde = beta + sum(k)^-1 * Winv %*% (colSums(S)-sum(k)*diag(PI))
...
sample_transaction = function(X,k,alpha, beta, cn) { 
  ...
  if (k>1) {
    for (i in 2:k) {
      ...
      if (is.null(nrow(X_new))) { # if X_new is a single row
        rsk = as.numeric(exp(X_new[1:cn] %*% alpha + X_new[cn + 1:n] %*% beta))
        } else {rsk = as.numeric(exp(matrix(X_new[, 1:cn], ncol = cn) %*% alpha + X_new[, cn + 1:n] %*% beta))}
...
# input: G, C, cn, nt, theta, beta (n tuple), alpha (depends on graph sample)
# output: S, D

lag_sample = function(G, C, cn, nt, theta, gamma, beta, alpha) {
  n = gorder(G)
  ...
  # sampling weights:
  sw <- drop(exp(array(X[, 1:cn], dim=c(nrow(X),cn)) %*% alpha + X[, cn + 1:n] %*% beta))
  sp <- sw / sum(sw)
  ...
  for (i in 1:nt) {
    ...
    S[i,] = st
    #true_cliques = list.append(true_cliques, st$cliques_chosen)
  }
  lhood.true = sum(S %*% c(alpha,beta) - k * log(sum(exp(X %*% c(alpha,beta)))) + k * log(theta))
  D = (t(apply(S,1,function(x) ifelse(x>0,1,0))))[,cn+1:n] # observed data
  return(list(D=D, lhood.true=lhood.true, k = k, S=S, Design = X))
}
